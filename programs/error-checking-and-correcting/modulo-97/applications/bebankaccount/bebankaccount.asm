...
; Build: see makefile
; Run:   ./bebankaccount n
;        n is a Belgian 12 digit bank account number.
; Description:
;        Modulo 97 check on bank account number.
...
section .data
        ...
        msgUsage:       db  "usage: bebankacc n",10,"n = 12 digit Belgian bank account number",10
        .length:        equ $-msgUsage
        msgResult:      db  "illegal bank account number", 10
...
_start:
        ...
        cmp     rcx, 14                 ; for the program we don't need the stringlength thus we check if length = 12 + 2
        jne     .usage
        xor     rdx, rdx                ; result
.repeat:        
...
.ascii:        
        ...
        add     rdx, rax
        cmp     rcx, 1                  ; last digit of bank account number?
        je      .done                   ; yes
...
.done:
        ...
        mov     rax, rdx
        mov     rsi, msgResult          ; point to 'illegal bankaccount number'
        mov     rdx, msgResult.length
        cmp     rax, rcx
        jne     .isnotequal
        inc     rsi                     ; point to 'legal bankaccount number'
        inc     rsi
